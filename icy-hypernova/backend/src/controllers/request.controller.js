...
const getRequests = async (req, res, next) => {
  try {
    ...
    if (user.role === 'receiver') {
      ...
      whereClause += ` AND (fr.volunteer_id = $${++paramIdx} OR fr.status = 'accepted')`;
      params.push(user.id);
    }
    ...
    const result = await query(
      `SELECT fr.*,
        ...
        u_volunteer.name as volunteer_name, u_volunteer.phone as volunteer_phone
       FROM food_requests fr
       JOIN food_donations fd ON fr.donation_id = fd.id
       JOIN users u_receiver ON fr.receiver_id = u_receiver.id
       JOIN users u_donor ON fd.donor_id = u_donor.id
       LEFT JOIN users u_volunteer ON fr.volunteer_id = u_volunteer.id
       ${whereClause}
...
const createRequest = async (req, res, next) => {
  try {
    ...
    const donationResult = await query(
      `SELECT * FROM food_donations WHERE id = $1 AND is_available = true AND expiry_time > NOW()`,
      [donation_id]
    );
    ...
    // Generate QR code
    const qrData = JSON.stringify({ request_id: uuidv4(), donation_id, receiver_id: req.user.id, ts: Date.now() });
    ...
    // Notify donor
    const donorResult = await query('SELECT id, name, email FROM users WHERE id = $1', [donation.donor_id]);
...
const updateRequestStatus = async (req, res, next) => {
  ...
  try {
    ...
    if (status === 'assigned') { updates.assigned_at = new Date(); updates.volunteer_id = volunteer_id; }
    ...
    if (status === 'assigned' && volunteer_id) {
      paramIdx++; updateQuery += `, volunteer_id = $${paramIdx}`; updateParams.push(volunteer_id);
      paramIdx++; updateQuery += `, assigned_at = $${paramIdx}`; updateParams.push(new Date());
    }
    ...
    // If accepted, mark donation unavailable if needed
    if (status === 'accepted') {
      await client.query(
        'UPDATE food_donations SET is_available = false WHERE id = $1',
        [request.donation_id]
      );
    }
    ...
    if (status === 'completed') {
      await client.query(
        'INSERT INTO deliveries (request_id, volunteer_id, status, actual_delivery_time) VALUES ($1, $2, $3, NOW()) ON CONFLICT DO NOTHING',
        [id, request.volunteer_id, 'delivered']
      );
    }
