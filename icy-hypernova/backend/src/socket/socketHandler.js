...
function initSocketIO(server) {
  ...
  io.on('connection', (socket) => {
    ...
    // Handle location update from volunteers
    socket.on('location_update', async (data) => {
      try {
        ...
        await query(
          `INSERT INTO location_tracking (delivery_id, volunteer_id, latitude, longitude)
           VALUES ($1, $2, $3, $4)`,
        ...
        if (deliveryResult.rows.length > 0) {
          const { receiver_id, donor_id } = deliveryResult.rows[0];
          
          // Broadcast location to receiver and donor
          const locationData = {
            delivery_id,
          ...
          io.to(`user_${receiver_id}`).emit('volunteer_location', locationData);
          io.to(`user_${donor_id}`).emit('volunteer_location', locationData);
        }
      } catch (error) {
