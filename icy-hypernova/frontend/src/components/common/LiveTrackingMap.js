...
const LiveTrackingMap = ({ 
  ...
  const path = useMemo(() => {
    const points = [];
    if (volunteerPos?.lat) points.push([volunteerPos.lat, volunteerPos.lng]);
    ...
    <div style={{ 
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <MapContainer center={mapCenter} zoom={15} style={{ height: '100%', width: '100%' }}>
          ...
          {donorPos?.lat && <Marker position={[donorPos.lat, donorPos.lng]} icon={donorIcon}><Popup>Donor Location</Popup></Marker>}
          {receiverPos?.lat && <Marker position={[receiverPos.lat, receiverPos.lng]} icon={receiverIcon}><Popup>Receiver Location</Popup></Marker>}
          {volunteerPos?.lat && <Marker position={[volunteerPos.lat, volunteerPos.lng]} icon={volunteerIcon}><Popup>Volunteer Location</Popup></Marker>}
