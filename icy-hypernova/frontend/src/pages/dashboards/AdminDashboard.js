  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4" style={{ marginBottom: '24px' }}>
        ...
        <StatCard icon="✅" label="Completed" value={stats?.completedRequests || 0} color="var(--success)" bg="var(--success-light)" change={15} />
        <StatCard icon="👥" label="Total Users" value={stats?.totalUsers || 0} color="var(--accent-yellow)" bg="var(--accent-yellow-light)" change={5} />
      </div>
      ...
      <div className="grid grid-cols-4" style={{ marginBottom: '28px' }}>
        ...
        <StatCard icon="🚚" label="Volunteers" value={stats?.totalVolunteers || 0} color="#0d9488" bg="#ccfbf1" />
        <StatCard icon="🟢" label="Active Donations" value={stats?.activeDonations || 0} color="var(--success)" bg="var(--success-light)" />
        <StatCard icon="📈" label="Efficiency" value={`${stats?.deliveryEfficiency || 0}%`} color="#f59e0b" bg="#fef3c7" />
      </div>
            ...
            <tbody>
              {recentUsers.map(u => (
                <tr key={u.id}>
                  ...
                  <td>
                    <span className={`badge ${u.role === 'admin' ? 'badge-primary' : u.role === 'donor' ? 'badge-warning' : u.role === 'volunteer' ? 'badge-teal' : 'badge-info'}`} style={{ textTransform: 'capitalize' }}>
                      {u.role}
                    </span>
                  </td>
